...
function CallbackHandler() {
  ...
  useEffect(() => {
    ...
    if (!consumeOAuthState("sentry", state)) {
    ...
    api.post("/api/settings/sentry/oauth", { 
      ...
      .catch((e: Error) => {
        setError(e.message || "Failed to connect to Sentry");
      });
  }, [code, state]);
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full space-y-4 p-8">
      ...
      <p className="text-text font-medium text-lg">Connecting Sentry...</p>
      <p className="text-text3 text-sm">Exchanging authorization code and fetching organization securely.</p>
    </div>
  );
}
