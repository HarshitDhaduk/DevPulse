...
function CallbackHandler() {
  ...
  useEffect(() => {
    ...
    if (!consumeOAuthState("linear", state)) {
      setError(OAUTH_STATE_ERROR);
      return;
    }

    api.post("/api/settings/linear/oauth", { 
      code,
      ...
      .catch((e: Error) => {
        setError(e.message || "Failed to connect to Linear");
      });
  }, [code, state]);
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full space-y-4 p-8">
      ...
      <p className="text-text font-medium text-lg">Connecting Linear...</p>
