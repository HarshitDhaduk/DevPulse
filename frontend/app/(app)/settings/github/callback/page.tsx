...
function CallbackHandler() {
  ...
  useEffect(() => {
    ...
    api.post("/api/settings/github/oauth", { code })
      .then(() => {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full space-y-4 p-8">
      <div className="w-12 h-12 rounded-full bg-devblue/20 flex items-center justify-center animate-pulse text-devblue">
        <svg className="w-6 h-6 animate-spin" fill="none" viewBox="0 0 24 24">
          <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
          <path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"></path>
        </svg>
      </div>
      <p className="text-text font-medium text-lg">Connecting GitHub...</p>
